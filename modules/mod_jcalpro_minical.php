...
if (!defined('EXTCALENDAR_CONFIG_SET')) {
	...
	$CONFIG_EXT['calendar_calling_page'] = $mosConfig_live_site . "/index.php?option=com_jcalpro";  // Your calendar web url
...
/* new in 1.5.0.2 */
$use_jcal_itemid = intval($params->get('use_component_itemid'));
if ($use_jcal_itemid)
{
	$jcal_itemid_mini = trim($params->get('component_itemid_mini'));
	if ($jcal_itemid_mini == "")
	{
		...
		$jcal_itemid_mini = $database->loadResult();
	}
	$CONFIG_EXT['calendar_calling_page'] = $mosConfig_live_site . "/index.php?option=com_jcalpro&Itemid=";
	$CONFIG_EXT['calendar_calling_page'] .= $jcal_itemid_mini ? $jcal_itemid_mini : $Itemid;  // Your calendar web url
}
...
if( !function_exists('minical_theme_mini_cal_view') )
{
  function minical_theme_mini_cal_view($date, &$results, &$info_data)
  {
    ...
    for ($i=1-$firstday;$i<=count($results);$i+=7)
    {
		...
		$params = array(
			'{ADD_EVENT_URL}' => $CONFIG_EXT['calendar_calling_page'] . "&amp;extmode=event&amp;event_mode=add",
...
if( !function_exists('minical_get_events') )
{
  ...
  while ($row = extcal_db_fetch_row($result))
  {
	...
	if($include_recurrent) {
	  ...
	  $query .= "WHERE (DATE_FORMAT(e.start_date,'%Y%m%d') <= $day_pattern) AND c.published = '1' AND e.published = '1' AND e.published = '1' AND approved = '1' AND recur_type <> '' $cat_filter ORDER BY start_date,title ASC";
	...
	<tr>
		<td align="center" nowrap="nowrap" class="extcal_navbar">
			<a href="{ADD_EVENT_URL}"
				...
				onmouseout="extcal_showOnBar('');return true;" style="display:block; border: 1px solid #BEC2C3" class="extcal_tiny_add_event_link"><img src="{THEME_DIR}/images/addsign_a.gif" style="vertical-align: text-bottom" alt="{ADD_EVENT_TITLE}" border="0" vspace="2" /> {ADD_EVENT_TITLE}</a>
		</td>
	</tr>
</table>
<!-- END add_event_row -->
