...
/*
**********************************************
JCal Pro Latest Events Module v1.5.3
...
require_once( $mainframe->getCfg('absolute_path')."/components/com_jcalpro/config.inc.php" );
...
if ( $EXTCAL_CONFIG['show_recurrent_events'] == 2 ) { // Show only first occurrence of recurring events:
  ...
  $rowsUpcomingRecurrentGross = $database->loadObjectList();
  $reccount = count($rowsUpcomingRecurrentGross);
  $count=0;
  while(  ( count($rowsUpcomingRecurrent) < $EXTCAL_CONFIG['number_of_events_to_list_upcoming'] ) &&
          ( $count < $reccount ) ) {
    $row = $rowsUpcomingRecurrentGross[$count];
	if ( has_priv ( 'category' . $row->cat ) ) {		
	  if ( $row->end_date == '0000-00-00 00:00:01' ) { // Now we strip out all day events which don't fall on today's date
	    // If this is an all day event, and it starts today, include it here
	    if ( strtotime(substr($row->start_date,0,10)) >= strtotime(substr($EXTCAL_CONFIG['now'],0,10)) )  $rowsUpcomingRecurrent[] = $row;
	  } else {
		$rowsUpcomingRecurrent[] = $row;
	  }
	}
    ++$count;
  }
...
if ($EXTCAL_CONFIG['number_of_events_to_list_recent']) {
  ...
  $calendarquery = "SELECT e.*,cat,cat_name from #__jcalpro_events AS e LEFT JOIN #__jcalpro_categories AS c ON e.cat=c.cat_id
	 WHERE ( ( UNIX_TIMESTAMP(e.end_date) < UNIX_TIMESTAMP('".$EXTCAL_CONFIG['now']."') )  OR  ( e.end_date = '0000-00-00 00:00:01' ) )
	  AND c.published = '1' AND e.published = '1' AND approved = '1' AND recur_type = ''"
	  . ( $catid ? "\n AND ( cat IN ( $catid ) )" : '' ) . "
	   ORDER BY e.start_date,e.title ASC";
...
if ( count($rowsUpcoming) ) {
	...
	$ext_event_link_URL = sefRelToAbs( 'index.php?option=com_jcalpro&amp;Itemid=' . $EXTCAL_CONFIG['Itemid'] . '&amp;extmode=view&amp;extid='. $thisEvent->extid );
	$ext_full_calendar_URL = sefRelToAbs( 'index.php?option=com_jcalpro&amp;Itemid=' . $EXTCAL_CONFIG['Itemid'] );
	// Actual output:
	if ( $outputSetting == 1)
	  {
	  	$module_output_array[$extcounter] .= '<li class="eventList">';
	  }
	else
	  {
	...
	$module_output_array[$extcounter] .= "<div class='eventDate'>". mf_get_daterange_string( $thisEvent )."</div>";
	$module_output_array[$extcounter] .= "<!-- <br /> --> \n";
...
if ( $EXTCAL_CONFIG['show_add_event_link'] && latest_has_priv($EXTCAL_CONFIG['who_can_add_events']) ) {
  $ext_add_event_URL = sefRelToAbs( 'index.php?option=com_jcalpro&amp;Itemid=' . $EXTCAL_CONFIG['Itemid'] . '&amp;extmode=event&amp;event_mode=add' );
