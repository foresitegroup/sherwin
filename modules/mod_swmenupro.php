...
$content= "\n<!--SWmenuPro5.1 ".$menustyle." by http://www.swonline.biz-->\n";   
...
if($menu && $id && $menustyle){
	...
	if(count($ordered)){
		if ($menustyle == "clickmenu"){$content.= doClickMenu($ordered, $swmenupro, $css_load,$active_menu);}
		...
		if ($menustyle == "gosumenu" && $editor_hack){$content.= doGosuMenu($ordered, $swmenupro, $active_menu, $css_load,$selectbox_hack);}
		if ($menustyle == "transmenu"){$content.= doTransMenu($ordered, $swmenupro, $active_menu, $sub_indicator, $parent_id, $css_load,$selectbox_hack,$show_shadow);}
		...
		if ($menustyle == "slideclick"){$content.= doSlideClick($ordered, $swmenupro, $css_load,$active_menu,$selectbox_hack);}
	}
}
