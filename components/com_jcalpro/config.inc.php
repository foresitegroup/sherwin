...
global $database, $mainframe, $my, $Itemid, $session;
...
// FS configuration
$CONFIG_EXT['FS_PATH'] = $mainframe->getCfg('absolute_path') . "/components/com_jcalpro/";        // Your file system path
$CONFIG_EXT['calendar_url'] = $mainframe->getCfg('live_site') . "/components/com_jcalpro/";        // Your calendar web url
...
$extmode = mosGetParam($_REQUEST, 'extmode', '');
$event_mode = mosGetParam($_REQUEST, 'event_mode', '');
$extid = mosGetParam($_REQUEST, 'extid', '');
if ($extid != '') {
	$extid = intval($extid);
}

$event_id = mosGetParam($_REQUEST, 'event_id', $extid);
$cat_id = mosGetParam($_REQUEST, 'cat_id', '');
if ($cat_id != '') {
	$cat_id = intval($cat_id);
}
$extcal_search = $database->getEscaped(mosGetParam($_POST, 'extcal_search', ''));
