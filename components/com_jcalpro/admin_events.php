...
// If not, we enter a special end_date instead.
		        ...
		        $template_vars = array(
					...
					// Successfull message
					$buttonURL = has_priv('Administrator') ? sefRelToAbs( $CONFIG_EXT['calendar_calling_page'].'&amp;extmode=event&amp;event_mode=view' ) : sefRelToAbs( $CONFIG_EXT['calendar_calling_page'] );
			...
			// Render the form
      if(!$successful) {
	  			$sef_href = sefRelToAbs( $CONFIG_EXT['calendar_calling_page'].'&amp;event_mode=edit' );
...
function print_event_list() {
	...
	while ($row = extcal_db_fetch_object($result))
	{
		...
		$sef_href = sefRelToAbs( $CONFIG_EXT['calendar_calling_page']."&amp;extmode=event&amp;event_mode=view&amp;extid=".$row->extid );
