...
function theme_admin_menu() {
  global $CONFIG_EXT, $database, $my, $lang_event_admin_data, $event_mode;
  $returnstring = '';
  if (has_priv("approve") && ($event_mode != 'view')) {
   ...
   // and if you are not already viewing those events. 
	...
	if ($rows > 0) {
	   ...
	   $sef_href = sefRelToAbs( $CONFIG_EXT['calendar_calling_page'].'&amp;extmode=event&amp;event_mode=view' );
...
{
	global $CONFIG_EXT, $database, $my, $THEME_DIR, $lang_event_admin_data, $event_mode;
...
function theme_cat_events_list(&$results, $cat_info, $stats) {
    global $template_cat_events_list, $database, $lang_cat_events_view, $today, $lang_date_format;
...
function theme_view_event(&$event, $is_popup = false) {
    ...
    global $template_event_view, $database, $my, $lang_event_view, $lang_general, $lang_date_format, $lang_add_event_view, $extmode, $CONFIG_EXT, $REFERER, $THEME_DIR;
		...
		$sef_href = sefRelToAbs( $CONFIG_EXT['calendar_calling_page']."&amp;extmode=event&amp;event_mode=edit&amp;extid=".$event->extid );
		...
		$sef_href = sefRelToAbs( $CONFIG_EXT['calendar_calling_page']."&amp;extmode=event&amp;event_mode=del&amp;extid=".$event->extid );
		...
		$sef_href = sefRelToAbs( $CONFIG_EXT['calendar_calling_page']."&amp;extmode=cat&amp;cat_id=".$event->catId );
