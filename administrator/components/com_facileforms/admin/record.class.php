...
class facileFormsRecord
{
	...
	function expcsv(&$recparams, $ids)
	{
		global $database, $ff_compath, $ff_version, $mosConfig_fileperms;

		$csvname = $ff_compath.'/exports/ffexport-'.date('YmdHis').'.csv';
		$existed = file_exists($csvname);
		if ($existed) {
			$permission = is_writable($csvname);
			if (!$permission) {
		...
		if (!$existed) {
			...
			if (isset($mosConfig_fileperms)) {
				...
				$filemode = 0644;
			if (isset($filemode)) @chmod($csvname, $filemode);
		} // if
		...
		facileFormsRecord::listitems($recparams, $csvname);
	} // expcsv
