...
function defaultTemplate ( )
{
	...
	mosRedirect('index2.php?option=com_jcalpro&task=showthemes');
}
...
function saveThemes( $option, $client, $task ) {
	...
	switch ( $task ) {
		case 'apply':
			...
			mosRedirect( 'index2.php?option='. $option .'&client='. $client .'&task=edittheme&hidemainmenu=1&id='. $row->id, $msg );
            break;
		case 'save':
		default:
			$msg = 'Successfully Saved Theme: '. $row->name;
			mosRedirect( 'index2.php?option='. $option .'&client='. $client .'&task=showthemes', $msg );
			break;
	}
}
...
function applyAccess( $cid=null, $option, $client ) {
	...
	mosRedirect( 'index2.php?option='. $option .'&client='. $client .'&task=showthemes' );
}
/**
...
function removeTheme( $cid, $option, $client ) {
	...
	mosRedirect( 'index2.php?option=com_jcalpro&task=remove&element=themes&client='. $client .'&cid[]='. $cid );
}
/**
...
function accessMenu( $uid, $access, $option ) {
	...
	mosRedirect( 'index2.php?option='. $option .'&task=showthemes' );
}
?>
